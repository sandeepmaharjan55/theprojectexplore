...
const PrivacyList = ({ theme }) => {
  ...
  const saveTerms = (event) => {
    ...
    //alert(data)
     axios.post(`${config.baseUrl}/privacypolicy/edit`, 
    data, {
    headers: {
   ...
   .then(function (response) {
        setModal(!modal)
        setReload(new Date())
        toast.success(<Toast title="Privacy Policy Updated" text="" type ="Success"/>, { autoClose: 3000, hideProgressBar: true })
   })
  ...
  useState(() => {
    axios
    .get(`${config.baseUrl}/privacypolicy/fetch`, {
      headers: {
