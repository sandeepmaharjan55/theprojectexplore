...
const Login = () => {
  ...
  .then(function (response) {
       ...
       toast.error(<Toast title="Welcome to INKS-VILLA" text="" type ="Info"/>, { autoClose: 3000, hideProgressBar: true })
     localStorage.setItem('userData', JSON.stringify(response.data));
     history.push('/home')
  })
  .catch(function (error) {
     if (error.response && error.response.status === 400) toast.success(<Toast title={error.response.data.message} text={error.response.data.errors.Error} type ="Error"/>, { autoClose: 3000, hideProgressBar: true })
     else toast.success(<Toast title="Some Error Occurred" text="" type ="Error"/>, { autoClose: 3000, hideProgressBar: true })
     setLoading(false)
  });
  }
   ...
   <h2 className='brand-text text-primary ml-1'>Admin Login</h2>
            ...
            <Form className='auth-login-form mt-2'  onSubmit={submitForm}>
              <FormGroup>
                ...
                <Input type='text' name="emailOrUsername" id='emailOrUsername' placeholder='emailOrUsername' autoFocus />
              </FormGroup>
              <FormGroup>
