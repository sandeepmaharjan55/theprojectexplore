...
const ReportedFeedManage = () => {
  ...
  const enableDesign = (parameter) => {

    const formData = new FormData();
     formData.append('id', parameter)
     formData.append('activeStatus', "true")
     axios.post(`${config.baseUrl}/products/brand/status`,
      formData, {
      headers: {
  ...
  const handleConfirmTextEnable = (id) => {
    return MySwal.fire({
      title: 'Are you sure you want to Enable the Design?',
      text: "",
      icon: 'info',
  ...
  const handleConfirmTextDisable = (id) => {
    return MySwal.fire({
      title: 'Are you sure you want to Disable the Design?',
      text: "",
      icon: 'info',
      showCancelButton: true,
 ...
 const columns = [
  ...
  useEffect(() => {
    axios
    .get(`${config.baseUrl}/products/brand/list`, {
      headers: {
    ...
    <Fragment>
      <Card>
        <CardHeader className='flex-md-row flex-column align-md-items-center align-items-start border-bottom'>
          <CardTitle tag='h4'>Verified Designs</CardTitle>
